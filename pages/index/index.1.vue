<template>
	<view class="page">
		<view class="content" id="content">
			<view class="popLayout" @touchmove.stop="" style="'height1:'+ height +'px;'" v-show="gameover_pop">
			...
			<view class="popLayout" @touchmove.stop="" v-show="popType_dashang">
				<view class="popContent">
					<view class="" style="flex: 1;"></view>
					<view class="popLayout_main">
						<view class="dashang_view">
							<view class="dashang_type">
								<label class="radio" @click="dashang_type='MT'"><radio :checked="dashang_type=='MT'"/>监版</label>
							...
							<view class="dashang_val" style="width: 100%;">
								...
								<label class="radio" @click="dashang_val=100"><radio color="red" :checked="dashang_val==100"/>100</label>
								<view style="width: 20px;height: 10px;"></view>
								<label class="radio" @click="dashang_val=300"><radio color="red" :checked="dashang_val==300"/>300</label>
								<view style="width: 20px;height: 10px;"></view>
								<label class="radio" @click="dashang_val=500"><radio color="red" :checked="dashang_val==500"/>500</label>
								<view style="flex: 1;"></view>
							</view>
							<view class="dashang_but">
								<button class="cancel"  type="primary" @click="popType_dashang=false;">取消</button>
			...
			<view id="videoLayout" style="height: 480upx;">
				直播模块区域
			</view>
			...
			<!-- <marquee class="marquee" direction="left" behavior="scroll" scrollamount="3">
				...
				<!-- 严正声明:斗蛐蛐作为娱乐项目,严禁参与一切赌博相关行为,如有发现作封号处理,望大家共创和谐平台! -->
			<my-marquee class="marquee" :lists="marqueelist">
			</my-marquee>
			<view class="titlelayout"> 
				<view class="titleL">
					<view class="left1">
						第{{dui?dui:0}}对&nbsp;(共{{total_dui?total_dui:0}}对)&nbsp;&nbsp;&nbsp;{{user_id}}({{user_score}})
					</view>
				</view>
				<view class="titleR">
					<view class="titleRBut" @click="popType_dashang=true;dashang_type='MT';dashang_val=100">打赏</view>
				</view>
			</view>
							...
							<view class="myLiL redBg" :class="ctr_li_index==index?'redBgSelect':''">
										...
										<view class="myli_item_r" >
											<view v-for="(itemI,i) in item[0].status" :key="i">
												<view v-show="itemI.jie" class="myli_item_r_content" @click="liClick(index,i,'red',item)">
													<view class="myli_item_title" :hidden="itemI.val==-1">
														{{itemI.val}}
													</view>
													<view class="myli_item_r_flex">
													</view>
													<view class="myli_item_but" :hidden="itemI.val==-1">
														<label class="mybut_jie">接</label> 
													</view>
												</view>
												<view v-show="itemI.fang" class="myli_item_r_content"  @click="liClick(index,i,'red',item)">
													<view class="myli_item_title" :hidden="itemI.val==-1"> 
														{{itemI.val}}
													</view>
													<view class="myli_item_r_flex">
							...
							<view class="myLiR blueBg" :class="ctr_li_index==index?'blueBgSelect':''">
										...
										<view class="myli_item_r" >
											<view v-for="(itemI,i) in item[1].status" :key="i">
												<view v-show="itemI.jie" class="myli_item_r_content" @click="liClick(index,i,'blue',item)">
													<view class="myli_item_title" :hidden="itemI.val==-1">
														{{itemI.val}}
													</view>
													<view class="myli_item_r_flex">
												...
												<view v-show="itemI.fang"  class="myli_item_r_content" @click="liClick(index,i,'blue',item)">
													<view class="myli_item_title" :hidden="itemI.val==-1">
														{{itemI.val}}
													</view>
													<view class="myli_item_r_flex">
...
import myMarquee from '@/components/my-marquee/my-marquee.vue'
	...
	var times = 1000;
	const datalist_init=[
					[{type:10,val:10,status:[{jie:0,fang:0,val:-1}]},{type:10,val:10,status:[{jie:0,fang:0,val:-1}]},{isCtr:0}],
					[{type:8,val:8,status:[{jie:0,fang:0,val:-1}]},{type:8,val:8,status:[{jie:0,fang:0,val:-1}]},{isCtr:0}],
					[{type:7,val:7,status:[{jie:0,fang:0,val:-1}]},{type:7,val:7,status:[{jie:0,fang:0,val:-1}]},{isCtr:0}],
					[{type:6,val:6,status:[{jie:0,fang:0,val:-1}]},{type:6,val:6,status:[{jie:0,fang:0,val:-1}]},{isCtr:0}],
					[{type:5,val:5,status:[{jie:0,fang:0,val:-1}]},{type:5,val:5,status:[{jie:0,fang:0,val:-1}]},{isCtr:0}],
					[{type:4,val:4,status:[{jie:0,fang:0,val:-1}]},{type:4,val:4,status:[{jie:0,fang:0,val:-1}]},{isCtr:0}]
				];
	...
	export default {
		data() {
			return {
				...
				height:400,
				liveUrl:'rtmp://139.159.221.9/zxc/live'
			}
		},
		watch: {
		...
		onReady() {
			var top=0;
			var view = uni.createSelectorQuery().select(".statusbar");
			view.boundingClientRect(data_ => {
				top = data_.height;
				var view = uni.createSelectorQuery().select("#videoLayout");
				view.boundingClientRect(data => {
				//console.log("节点离页面顶部的距离为" + data.height);
					this.getLiveUrl(top,data.height);
				}).exec();
			}).exec();
					...
					success: res => {
						if(res.data.status=='1')
						{
							...
							if(JSON.stringify(data_) != "{}")
							{
								...
								// 设置红方押注信息
								this.setYaZhuList(data_, 'red');
								// 设置蓝方押注信息
								this.setYaZhuList(data_, 'blue');
