<template>
	<view class="body">
		<view class="r-item" v-for="(item,index) in lists" :key="index">
			<view class="header">
				<view>会员:{{item.nick_name}}</view>
				<view>{{item.created_time}}</view>
				<view>{{item.state == 1 ? '上分':'下分'}}</view>
			</view>
			<view class="table">
				<view class="tr">
					<view class="td">操作积分</view>
					<view class="td">开始积分</view>
					<view class="td">结束积分</view>
				</view>
				<view class="tr">
					<view class="td">{{item.point}}</view>
					<view class="td">{{item.point_start}}</view>
					<view class="td">{{item.point_end}}</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			getList(current, size, callback) {
				let that = this
				this.bus.request({
					url: this.bus.url + '/point/agentFindUserChargePointList',
					method: 'GET',
					data: {
